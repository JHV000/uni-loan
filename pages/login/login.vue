<template>
	<view class="page">
		...
		<view class="login-box">
			...
			<view class="login-card-loginIn"><view class="login-card-loginIn-btn" @click="submitForm">登 录</view></view>
			<navigator url="/pages/register/register" hover-class="navigator-hover"><view class="login-bottom-info">新员工请先注册</view></navigator>
		</view>
	</view>
</template>
				...
				.then(res => {
					...
					api.login(res).then(result => {
						...
						if (result.statusCode == 200) {
							...
							if (code == 1) {
								var access_token = result.data.result.data.access_token;
								uni.showToast({
									title: '登录成功',
									icon: 'success'
								});
								uni.setStorageSync('login_status', { access_token: access_token, username: this.formData.name, password: this.formData.password });
								// var dd = uni.getStorageSync('login_status')
								// console.log(dd)
								uni.redirectTo({
									url: '../index/index'
								});
